...
def sample_lines(text: str, pattern: re.Pattern, max_lines: int = 3) -> List[str]:
    out: List[str] = []
    for line in (text or "").splitlines():
        if pattern.search(line):
            out.append(redact_secret(line.strip()))
        if len(out) >= max_lines:
            break
    return out


def scan_text_for_exposures(location: str, text: str, content_type: str = "") -> List[ExposureFinding]:
    ...
    for name, pattern, confidence, value in SECRET_PATTERNS:
        ...
        if key in seen:
            continue
        seen.add(key)
        evidence = "; ".join(redact_secret(m.group(0)) for m in matches[:3])
    ...
    for name, pattern in STORAGE_PATTERNS:
        ...
        findings.append(ExposureFinding(
            ...
            confidence=conf,
            location=location,
            evidence="; ".join(redact_secret(u) for u in urls[:5]),
    ...
    for name, pattern in ERROR_SIGNATURES:
        if pattern.search(body):
            findings.append(ExposureFinding(
                ...
                confidence=78,
                location=location,
                evidence="; ".join(sample_lines(body, pattern, 2)) or name,
...
def build_data_exposure_report(blobs: Dict[str, str], content_types: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
    ...
    for location, text in blobs.items():
        ...
        findings.extend(scan_text_for_exposures(location, text, ctype))
