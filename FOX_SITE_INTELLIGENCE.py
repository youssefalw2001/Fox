...
def main() -> None:
    ...
    parser.add_argument("--unfiltered", action="store_true", help="JUDGE MODE: Disable output redaction for authorized review - shows full secrets, tokens, and response bodies")
    ...
    if args.unfiltered:
        print("\n" + "=" * 70)
        print("⚠️  UNFILTERED MODE ACTIVE - AUTHORIZED JUDGE REVIEW")
        print("=" * 70)
        print("• Secrets, tokens, and credentials will NOT be redacted")
        print("• Full HTTP response bodies will be stored")
        print("• This mode is for authorized security review ONLY")
        print("=" * 70 + "\n")
    ...
    if args.data_intel:
        data_exposure = build_data_exposure_report(blobs or {}, content_types or {}, unfiltered=args.unfiltered)
    ...
    if args.judge_mode:
        result["judge_review"] = build_judge_review_report(result, full_evidence=args.unfiltered)
